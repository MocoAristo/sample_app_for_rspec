    ...
    describe 'タスク新規登録' do
      context 'フォームの入力値が正常' do
        it 'タスクの新規作成が成功する' do
          ...
          fill_in 'Deadline', with: DateTime.new(2021,1,1,12,20)
          ...
          expect(page).to have_content('Deadline: 2021/1/1 12:20')
      ...
      context '登録済のタイトルを入力' do
        it 'タスクの新規作成が失敗する' do
          task_with_duplicated_title = create(:task)
          click_link 'New Task'
          fill_in 'Title', with: task_with_duplicated_title.title
          fill_in 'Content', with: 'content'
          select 'doing', from: 'Status'
          fill_in 'Deadline', with: DateTime.new(2021,1,1,12,20)
          click_button 'Create Task'
